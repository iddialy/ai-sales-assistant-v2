...
def _normalize_tz_phone(value: str | None) -> str:
...
def _find_whatsapp_merchant(db: Session, phone_number_id: str | None = None) -> Merchant | None:
    if phone_number_id:
        row = (
            db.query(SocialConnection)
            .filter(
                SocialConnection.provider == "whatsapp",
                SocialConnection.account_id == str(phone_number_id),
                SocialConnection.status == "CONNECTED",
            )
            .first()
        )
        if row:
            return db.query(Merchant).filter(Merchant.user_id == row.merchant_id).first()

    fallback_id = os.getenv("WHATSAPP_MERCHANT_ID", "").strip()
    if fallback_id:
        return db.query(Merchant).filter(Merchant.user_id == fallback_id).first()

    return None


def _get_whatsapp_access_token(db: Session, merchant: Merchant) -> str | None:
    row = (
        db.query(SocialConnection)
        .filter(
            SocialConnection.merchant_id == merchant.user_id,
            SocialConnection.provider == "whatsapp",
            SocialConnection.status == "CONNECTED",
        )
        .first()
    ...
    if row:
        token = _decrypt_social_token(row.access_token_encrypted)
        if token:
            return token

    return os.getenv("WHATSAPP_ACCESS_TOKEN", "").strip() or None


def _send_whatsapp_text(phone_number_id: str, recipient: str, message: str, access_token: str):
...
def whatsapp_webhook_verify(
    ...
    if not verify_token:
        raise HTTPException(
            status_code=503,
            detail="WHATSAPP_VERIFY_TOKEN haijawekwa kwenye Render.",
        )
...
async def whatsapp_webhook(
    ...
    if app_secret:
        if not signature.startswith("sha256="):
            raise HTTPException(status_code=401, detail="WhatsApp webhook signature haipo.")
        expected = hmac.new(
        ...
        if not hmac.compare_digest(signature[7:], expected):
            raise HTTPException(status_code=401, detail="WhatsApp webhook signature si sahihi.")

    try:
        body = json.loads(raw_body.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError):
        raise HTTPException(status_code=400, detail="WhatsApp webhook payload si JSON sahihi.")
    ...
    for entry in body.get("entry") or []:
        for change in entry.get("changes") or []:
            ...
            merchant = _find_whatsapp_merchant(db, phone_number_id)
            if not merchant:
                errors.append("WhatsApp merchant hajapatikana.")
                continue

            access_token = _get_whatsapp_access_token(db, merchant)
            if not access_token:
                errors.append("WhatsApp access token haijapatikana.")
                continue

            contacts = value.get("contacts") or []
            contact_by_wa_id = {
                str(c.get("wa_id")): c for c in contacts if c.get("wa_id")
            }
            ...
            for message in value.get("messages") or []:
                ...
                from_phone = _normalize_tz_phone(message.get("from"))
                text_body = str((message.get("text") or {}).get("body") or "").strip()
                if not from_phone or not text_body:
                    continue

                contact = contact_by_wa_id.get(str(message.get("from"))) or {}
                profile = contact.get("profile") or {}
                customer_name = str(profile.get("name") or "WhatsApp Customer").strip()

                customer = (
                    db.query(Customer)
                    .filter(
                        Customer.merchant_id == merchant.user_id,
                        Customer.platform == "whatsapp",
                        Customer.external_key == from_phone,
                    )
                    .first()
                )
                ...
                try:
                    reply = generate_ai_sales_response(merchant, text_body, "whatsapp")
                    if not reply:
                        reply = "Samahani, kwa sasa sijaweza kutoa jibu. Tafadhali jaribu tena."
                    ...
                    db.rollback()
                    detail = exc.read().decode("utf-8", errors="replace")
                    errors.append(f"WhatsApp send error: {detail[:300]}")
                except Exception as exc:
                    db.rollback()
